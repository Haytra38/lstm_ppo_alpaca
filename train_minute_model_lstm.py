...
class MinuteTradingEnvHistorical(gym.Env):
    ...
    def __init__(self, df, initial_balance=1000.0, lookback_periods=30, lstm_model_path=None, lstm_scaler_path=None, use_lstm_features=True):
        ...
        self.ppo_integration = None
        self.lstm_features_size = 6 if use_lstm_features else 0  # 6 features LSTM pour PPO
        
        if lstm_model_path and os.path.exists(lstm_model_path):
            self._load_lstm_model(lstm_model_path, lstm_scaler_path)
        
        # Espace d'observation étendu pour inclure les données historiques + features LSTM
        # 5 features OHLCV par période * lookback_periods + features LSTM (6 dimensions)
        base_obs_size = 5 * lookback_periods
        lstm_obs_size = self.lstm_features_size if self.lstm_predictor else 0
        obs_size = base_obs_size + lstm_obs_size
        ...
        print(f"📊 Taille de l'observation: {obs_size} features (base: {base_obs_size}, LSTM: {lstm_obs_size})")
        ...
        if self.lstm_predictor:
            print(f"🧠 Modèle LSTM intégré: {lstm_model_path}")
            print(f"   📏 Longueur de séquence: {self.lstm_predictor.sequence_length}")
            print(f"   🔮 Features LSTM: {self.lstm_features_size}")
        else:
            print("⚠️  Pas de modèle LSTM chargé")
    
    def _load_lstm_model(self, model_path, scaler_path=None):
        """Charge un modèle LSTM pré-entraîné"""
        try:
            # Utiliser le nouveau prédicteur LSTM
            self.lstm_predictor = LSTMPredictor(model_path)
            self.ppo_integration = PPOIntegration(self.lstm_predictor)
            
            # Récupérer les métadonnées du modèle
            self.lstm_sequence_length = self.lstm_predictor.sequence_length
            self.lstm_nombre_predictions = self.lstm_predictor.nombre_de_predictions
            self.lstm_nombre_colonnes = self.lstm_predictor.nombre_de_colonnes
            
            print(f"✅ Modèle LSTM chargé avec succès depuis: {model_path}")
            print(f"   📏 Longueur de séquence: {self.lstm_sequence_length}")
            print(f"   🔮 Nombre de prédictions: {self.lstm_nombre_predictions}")
            print(f"   📊 Nombre de colonnes: {self.lstm_nombre_colonnes}")
            
        except Exception as e:
            print(f"❌ Erreur lors du chargement du modèle LSTM '{model_path}': {str(e)}")
            self.lstm_predictor = None
            self.ppo_integration = None
    
    def _get_lstm_predictions(self):
        """Génère les prédictions LSTM basées sur les données actuelles"""
        if not self.lstm_predictor or not self.ppo_integration or self.current_step < 30:
            return np.zeros(self.lstm_features_size, dtype=np.float32)
        
        try:
            # Préparer les données pour la prédiction LSTM
            # Utiliser toutes les colonnes disponibles pour correspondre au modèle entraîné
            available_columns = self.df.columns.tolist()
            
            # Prendre les colonnes OHLCV si disponibles, sinon utiliser ce qui est disponible
            if 'Volume' in available_columns:
                target_columns = ['Open', 'High', 'Low', 'Close', 'Volume']
                temp_df = self.df
            else:
                target_columns = ['Open', 'High', 'Low', 'Close']
                # Ajouter une colonne Volume factice si le modèle attend 5 colonnes
                if self.lstm_nombre_colonnes == 5 and len(target_columns) == 4:
                    # Créer une colonne Volume temporaire avec des valeurs moyennes
                    temp_df = self.df.copy()
                    temp_df['Volume'] = temp_df['Close'] * 1000  # Volume factice
                    target_columns.append('Volume')
                else:
                    temp_df = self.df
            
            # Prendre les données nécessaires pour la séquence LSTM
            start_idx = max(0, self.current_step - self.lstm_sequence_length + 1)
            end_idx = self.current_step + 1
            lstm_data = temp_df.iloc[start_idx:end_idx]
            
            # Vérifier que nous avons assez de données
            if len(lstm_data) < self.lstm_sequence_length:
                print(f"⚠️ Pas assez de données pour la prédiction LSTM: {len(lstm_data)} < {self.lstm_sequence_length}")
                return np.zeros(self.lstm_nombre_predictions * self.lstm_nombre_colonnes, dtype=np.float32)
            
            # Configuration de prédiction
            predict_config = {
                "target_columns": target_columns[:self.lstm_nombre_colonnes],  # Limiter au nombre de colonnes du modèle
                "prediction_steps": self.lstm_nombre_predictions,
                "confidence_interval": False
            }
            
            # Informations temporelles
            timeframe_info = {
                "end_date": lstm_data.index[-1] if hasattr(lstm_data.index, 'to_pydatetime') else None,
                "pandas_freq": "min"  # Minute
            }
            
            # Effectuer la prédiction avec le nouveau prédicteur
            predictions_result = self.lstm_predictor.predict(lstm_data, predict_config, timeframe_info)
            
            if "error" in predictions_result:
                print(f"⚠️ Erreur de prédiction LSTM: {predictions_result['error']}")
                return np.zeros(self.lstm_nombre_predictions * self.lstm_nombre_colonnes, dtype=np.float32)
            
            # Extraire et normaliser les prédictions
            future_predictions = predictions_result.get("future", {})
            predictions_array = []
            
            current_price = self.df.iloc[self.current_step]['Close']
            
            for col in target_columns:
                if col in future_predictions:
                    col_predictions = future_predictions[col]
                    # Normaliser par rapport au prix actuel
                    normalized_predictions = [(pred - current_price) / current_price if current_price > 0 else 0 
                                            for pred in col_predictions]
                    predictions_array.extend(normalized_predictions)
                else:
                    # Remplir avec des zéros si la colonne n'est pas disponible
                    predictions_array.extend([0.0] * self.lstm_nombre_predictions)
            
            # S'assurer que la taille est correcte
            expected_size = self.lstm_nombre_predictions * self.lstm_nombre_colonnes
            if len(predictions_array) > expected_size:
                predictions_array = predictions_array[:expected_size]
            elif len(predictions_array) < expected_size:
                predictions_array.extend([0.0] * (expected_size - len(predictions_array)))
            
            return np.array(predictions_array, dtype=np.float32)
            
        except Exception as e:
            print(f"⚠️ Erreur lors de la génération des prédictions LSTM: {str(e)}")
            return np.zeros(self.lstm_features_size, dtype=np.float32)
    ...
    def _get_observation(self):
        ...
        # Ajouter les prédictions LSTM si disponibles
        if self.lstm_predictor and self.ppo_integration:
            lstm_predictions = self.ppo_integration.get_state_features(self.df.iloc[max(0, self.current_step-30):self.current_step+1])
            lstm_predictions = np.nan_to_num(lstm_predictions, nan=0.0, posinf=1.0, neginf=-1.0)
            obs = np.concatenate([obs, lstm_predictions])
...
def train_minute_model_historical(symbol="BTC-USD", period="7d", interval="1m", timesteps=50000, 
                                 model_name=None, continue_training=False, lookback_periods=30, filename=None, use_file=False, lstm_model_name=None):
    """
    ...
    # Créer l'environnement avec analyse historique et modèle LSTM
    print(f"\n🏗️ Création de l'environnement de trading avec analyse historique...")
    env = MinuteTradingEnvHistorical(df, lookback_periods=lookback_periods, lstm_model_name=lstm_model_name)
    ...
    if continue_training and model_name:
        ...
        if loaded_lookback_periods is not None:
            ...
            # Recréer l'environnement avec les bons lookback_periods et le modèle LSTM
            env = MinuteTradingEnvHistorical(df, initial_balance=10000.0, lookback_periods=lookback_periods, lstm_model_name=lstm_model_name)
            env = DummyVecEnv([lambda: env])
...
def get_user_input_for_training():
    ...
    """
    print("\n🎯 Configuration de l'entraînement PPO avec analyse historique")
    print("=" * 60)
    
    # Choix entre données en ligne et fichier local
    print("\n📊 Source des données:")
    print("  1. 📈 Télécharger des données en ligne (yfinance)")
    print("  2. 📁 Utiliser un fichier de données local")
...
def select_lstm_model_for_ppo():
    """
    Interface pour sélectionner un modèle LSTM pré-entraîné pour le PPO
    """
    print("\n🧠 Configuration du modèle LSTM pour PPO")
    print("=" * 50)
    ...
    try:
        ...
        if not available_models:
            ...
            print("💡 Vous pouvez entraîner un modèle LSTM d'abord ou continuer sans modèle LSTM")
            
            while True:
                choice = input("\n➤ Continuer sans modèle LSTM ? (o/n): ").strip().lower()
                if choice in ['o', 'oui', 'y', 'yes']:
                    return None
                elif choice in ['n', 'non', 'no']:
                    print("\n🔄 Veuillez d'abord entraîner un modèle LSTM")
                    exit(1)
                else:
                    print("❌ Veuillez répondre par 'o' (oui) ou 'n' (non)")
        
        print(f"\n📋 {len(available_models)} modèle(s) LSTM disponible(s):")
        print("  0. ❌ Continuer sans modèle LSTM")
        ...
        while True:
            try:
                choice = int(input(f"\n➤ Sélectionnez un modèle (0-{len(available_models)}): "))
                
                if choice == 0:
                    print("\n✅ Entraînement PPO sans modèle LSTM")
                    return None
                    ...
                    try:
                        ...
                        print(f"\n✅ Modèle LSTM sélectionné: {selected_model}")
                        ...
                        print(f"\n❌ Erreur lors du chargement du modèle {selected_model}: {e}")
                        print("💡 Essayez un autre modèle ou continuez sans modèle LSTM")
                        continue
                        
                else:
                    print(f"❌ Numéro invalide. Entrez un nombre entre 0 et {len(available_models)}")
                    
            except ValueError:
                print("❌ Veuillez entrer un nombre valide")
                
    except Exception as e:
        print(f"❌ Erreur lors de la recherche des modèles: {e}")
        print("💡 Continuons sans modèle LSTM")
        return None

def select_model_for_training():
    """
    Interface pour sélectionner un modèle PPO existant ou créer un nouveau
    """
...
if __name__ == "__main__":
    ...
    if args.interactive:
        ...
        # Sélection du modèle LSTM pour le PPO
        lstm_model_name = select_lstm_model_for_ppo()
        
        # Sélection du modèle PPO
        ...
        use_file = False
        filename = None
        lstm_model_name = None  # Pas de modèle LSTM en mode ligne de commande
